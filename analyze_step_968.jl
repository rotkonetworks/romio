#!/usr/bin/env julia
# Analyze instruction at PC=0x32cf (step 968) that changes r6
        ...
        for preimage in acc[:data][:preimages_blob]
            if length(preimage[:blob]) > 10000
                ...
                if result !== nothing
                    ...
                    # From trace at step 967: r6=67848
                    r6_before = 67848
                    r6_after = (r6_before + immx) % 2^32
                    println("\nExecution:")
                    println("  r$ra ← r$rb + immx")
                    println("  r$ra ← $r6_before + $immx = $r6_after")
                end
                break
            end
        end
