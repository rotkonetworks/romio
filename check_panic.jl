        ...
        for preimage in acc[:data][:preimages_blob]
            if length(preimage[:blob]) > 1000
                ...
                if result !== nothing
                    ...
                    println("PC=0x$(string(pc, base=16)): opcode=$opcode skip=$skip ra=$ra rb=$rb imm=$immx_signed")

                    # r1 at step 9 was 0xFEFD0200
                    r1 = UInt64(0xFEFD0200)
                    addr = UInt64(Int64(r1) + immx_signed)
                    println("If rb=r1=0x$(string(r1, base=16)), addr = 0x$(string(addr, base=16))")

                    # Check if this address is in the accessible range
                    page = div(addr, 4096)
                    println("Page = $page (0x$(string(page, base=16)))")

                    # The first 64KB (pages 0-15) are always inaccessible
                    println("Is page < 16? $(page < 16) -> would cause PANIC")
                end
                break
            end
        end
